...
\begin{itemize}
    ...
    \cite{boeing777,boeingAirportPlanning},
...
The construction path is deliberately staged. Geometry and mass define the
reference quantities. NASA CRM data provide the active clean-cruise baseline.
The analytical seed remains available as a complete fallback. Digital DATCOM
Mach-grid, control-grid and derived gap-fill outputs are stored as candidate
derivative data until validation gates allow promotion. CRM-HL/DATCOM-ready
increments provide the first approach and landing configuration layer. The key
rule is that source data, candidate data and active plant data are not treated
as the same thing.
...
The plant applies command signals through actuator and engine dynamics rather
than feeding commanded values directly into the force and moment equations. The
current control-surface actuator model covers elevator, effective aileron and
rudder deflections. Each commanded deflection is first limited to the available
surface range:
\begin{equation}
    \delta_{cmd,lim} =
    \operatorname{sat}_{\delta_{min},\delta_{max}}
    \left(\delta_{cmd}\right).
\end{equation}
The actual surface state then follows a rate-limited first-order lag,
\begin{equation}
    \dot{\delta} =
    \operatorname{sat}_{\dot{\delta}_{min},\dot{\delta}_{max}}
    \left(\frac{\delta_{cmd,lim}-\delta}{\tau_{\delta}}\right),
\end{equation}
with position limits
\begin{equation}
    \delta_{min} \leq \delta \leq \delta_{max}.
\end{equation}
If a surface is already at a position limit, the rate command is prevented
from driving it farther outside the admissible range. The first implementation
uses engineering estimates of the limits and time constants: elevator
$\pm 25^\circ$, effective aileron $\pm 20^\circ$ and rudder $\pm 30^\circ$.
The actuator output passed to the aerodynamic model is the current actual
deflection, not the commanded value. This separation is required for NMPC
because the controller must respect both surface deflection limits and surface
rate limits.
