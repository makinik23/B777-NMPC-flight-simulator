...
The project defines representative reference cases in \texttt{reference\_cases.m}. The first case is a nominal cruise condition at approximately 35000 ft, or \SI{10668}{\metre}. The second case is an approach condition at approximately 3000 ft, or \SI{914.4}{\metre}, and a negative flight-path angle. Each case stores mass in kilograms and uses the fixed project gravity value. This avoids ambiguity associated with symbolic mass-case names and keeps the trim problem tied to explicit physical parameters.
...
\begin{itemize}
    \item sign checks for aerodynamic derivatives and control effects,
    \item static stability checks in longitudinal and lateral-directional channels,
    \item trim residual checks for each reference case,
    \item open-loop response checks for elevator, aileron, rudder and throttle perturbations,
    \item comparison of modal behaviour with expected transport-aircraft tendencies,
    \item sensitivity checks for mass and configuration changes.
\end{itemize}

\section{Linearisation Around Trim}
